...
# 	if i % 1 == 0:
# 		print(i)
for i in range(len(range_inh)):
	FRout_inh.append([])
	Adapt.append([])
	muvi.append([])
	Fe_eff[i][:] = range_exc # we try it with this scaling
	e=1 # we start at fe=!0	print("rate exc =", rate_exc)
	rate_inh = range_inh[i]
	while (e<JUMP.size):
	...
	# for rate_exc in range_exc:
		...
		mean_FR = mean(popRateG_inh[int(0.3*len(popRateG_inh))::])

		# FRout_inh[i].append(mean_FR)
		# Adapt[i].append(mean(uu[int(0.3*len(uu)):len(uu)]))
		# muvi[i].append(mean(vvi[int(0.3*len(vvi)):len(vvi)]))

		if (mean_FR>JUMP[e-1]): # if we make a too big jump
			...
			print("== the excitation level :", e+1," over ",len(range_exc))
			print("== ---- the inhibition level :", i+1," over ",len(range_exc))
			FRout_inh[i].append(mean_FR)
			Adapt[i].append(mean(uu[int(0.3*len(uu)):len(uu)]))
			muvi[i].append(mean(vvi[int(0.3*len(vvi)):len(vvi)]))

			if e<len(range_exc)-1: # we set the next value to the next one...
...
#
np.save(f'{save_path}data/ExpTF_{CELLS}_adapt_{Npts_e}x{Npts_i}_{save_name}.npy', Adapt)
np.save(f'{save_path}data/ExpTF_{CELLS}_fr_{Npts_e}x{Npts_i}_{save_name}.npy', FRout_inh)
np.save(f'{save_path}data/ExpTF_{CELLS}_muvi_{Npts_e}x{Npts_i}_{save_name}.npy', muvi)
...
np.save(f'{save_path}data/params_range_{Npts_e}x{Npts_i}_{save_name}.npy', np.array([Fe_eff, range_inh, params], dtype='object'), allow_pickle=True)
