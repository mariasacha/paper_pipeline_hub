...
def prepare_surface_regions_human(parameters, conn_filename='Connectivity.zip', zip_filename= 'Surface_Cortex.zip', region_map_filename = 'RegionMapping.txt'):
    ...
    # path of the connectivity file 
    path = parameters.parameter_connection_between_region['path']
    
    # connectivity of human TVB
    conn = connectivity.Connectivity().from_file(path+'/'+conn_filename)
    #conn.configure()
    
    # surface of human TVB
    ...
    # region mapping data
    region_mapping_data = np.loadtxt(path+'/'+region_map_filename, dtype=np.int)
    ...
    region_mapping = rm.RegionMapping(array_data=region_mapping_data, connectivity=conn, surface=surface_cortex)
    cortex = ct.Cortex().from_file()
    cortex.region_mapping_data = region_mapping
    
    return cortex, conn, hemispheres_left, hemispheres_right
